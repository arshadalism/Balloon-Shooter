...
pygame.display.set_caption('Arshad_Balloon Shooter')
...
def pointer():
    ...
    for i in range(noBalloon):
        if onBalloon(balloons[i].x, balloons[i].y, balloons[i].a, balloons[i].b, pos):
            color = red
    # Below lines are explaining the shooter structure
    ...
    pygame.draw.line(display, color, (pos[0] + l / 2, pos[1]), (pos[0] + l, pos[1]),4)  # 4 represents the line that is at the shooter
    pygame.draw.line(display, color, (pos[0], pos[1] + l / 2), (pos[0], pos[1] + l), 4)
    ...
    while loop:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_q:  # press q for quit
                    close()
                if event.key == pygame.K_r:  # press r for replay from start
                    score = 0
                    game()
        ...
        for i in range(noBalloon):  # This part of code is used to show the balloon on the screen
            balloons[i].show()

        pointer()

        for i in range(noBalloon):  # This is just moving the balloons across the screen
            balloons[i].move()
